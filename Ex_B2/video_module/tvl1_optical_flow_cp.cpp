...
{
    return !cvIsNaN(u.x) && !cvIsNaN(u.y) 
		&& fabs(u.x) < 1e9 && fabs(u.y) < 1e9;
}
...
static Vec3b computeColor(float fx, float fy)
{
    ...
    // 顏色轉移相對長度: 
	// 這些顏色被選是基於認知的相似性(perceptual similarity)
    // 例如. 紅與黃比黃與綠好區分色度(shades)
    const int RY = 15;
    const int YG = 6;
    const int GC = 4;
    const int CB = 11;
...
static void drawOpticalFlow(const Mat_<Point2f>& flow,
